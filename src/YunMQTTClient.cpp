...
boolean YunMQTTClient::begin() { return this->begin(""); }
...
void YunMQTTClient::setWill(const char *topic) { this->setWill(topic, ""); }
...
void YunMQTTClient::setTls(const char *caCerts) { this->tlsCaCerts = caCerts; }

boolean YunMQTTClient::connect(const char *clientId) {
  return this->connect(clientId, "", "");
}

boolean YunMQTTClient::connect(const char *clientId, const char *username,
                               const char *password) {
...
void YunMQTTClient::publish(String topic) { this->publish(topic.c_str(), ""); }
...
void YunMQTTClient::publish(const char *topic, char *payload,
                            unsigned int length) {
...
void YunMQTTClient::subscribe(String topic) { this->subscribe(topic.c_str()); }
...
void YunMQTTClient::loop() {
  ...
  if (av > 0) {
    ...
    if (ret.startsWith("m")) {
      ...
      int payloadLength =
          ret.substring(startPayloadLength, endPayloadLength).toInt();
