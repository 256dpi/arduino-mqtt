...
boolean MQTTClient::begin(Client &client) { return this->begin("", client); }
...
boolean MQTTClient::begin(const char *_hostname, int _port, Client &_client) {
  this->client =
      new MQTT::Client<Network, Timer, MQTT_BUFFER_SIZE, 0>(this->network);
...
void MQTTClient::setWill(const char *topic) { this->setWill(topic, ""); }
...
boolean MQTTClient::connect(const char *clientId, const char *username,
                            const char *password) {
...
boolean MQTTClient::publish(const char *topic, char *payload,
                            unsigned int length) {
...
boolean MQTTClient::connected() { return this->client->isConnected(); }
